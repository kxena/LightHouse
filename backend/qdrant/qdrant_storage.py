...
class QdrantManager:
    ...
    def create_collection(self, recreate: bool = False):
        ...
        # Index for disaster_type (keyword)
        ...
        # Index for is_disaster (boolean)
        self.client.create_payload_index(
            collection_name=self.collection_name,
            field_name="is_disaster",
            field_schema=PayloadSchemaType.BOOL
        )
        
        # Index for confidence (float) - for range queries
        self.client.create_payload_index(
            collection_name=self.collection_name,
        ...
        # Index for location (keyword) - optional but useful
        self.client.create_payload_index(
            collection_name=self.collection_name,
            field_name="location",
            field_schema=PayloadSchemaType.KEYWORD
        ...
        print(f"✓ Created collection '{self.collection_name}' with indexes (vector size: {vector_size})\n")
    ...
    def get_stats(self) -> Dict:
        ...
        disaster_types = {}
        for dtype in ['earthquake', 'flood', 'hurricane', 'wildfire']:
            results = self.client.scroll(
                collection_name=self.collection_name,
                scroll_filter=Filter(
                    must=[
